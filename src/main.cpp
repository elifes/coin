...
#if defined(NDEBUG)
# error "eLifescoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x84f55c9ead4352830daafc6421b055f9da0293e85e6fa22044295be839a10c1d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // eLifescoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "eLifescoin Signed Message:\n";
...
{
    // eLifescoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetSpacing = 1 * 30; // eLifescoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // eLifescoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    if(pindex->pprev)
    {
        pvHash = pindex->pprev->GetBlockHash();
    }

    if (vtx[0].GetValueOut() > GetBlockValue(pindex->nHeight, nFees, pvHash))
        return state.DoS(100, error("ConnectBlock() : coinbase pays too much (actual=%"PRI64d" vs limit=%"PRI64d")", vtx[0].GetValueOut(), GetBlockValue(pindex->nHeight, nFees, pvHash)));
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // eLifescoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x84f55c9ead4352830daafc6421b055f9da0293e85e6fa22044295be839a10c1d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "We are ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1523166458;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2519588;
        ...
        assert(block.hashMerkleRoot == uint256("0x21e5bd5e0aa0b5f4884eeceb9451a953f3af7940c612a030c0de6f8f1d0787d9"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // eLifescoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("eLifescoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("eLifescoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static eLifescoinMiner(CWallet *pwallet)
{
    printf("eLifescoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("elifescoin-miner");
    ...
    try { loop {
        ...
        printf("Running eLifescoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("eLifescoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&eLifescoinMiner, pwallet));
}
